...
class RoomFlowTests(TestCase):
    ...
    def test_narrator_dashboard_routes_each_admin_action(self):
        ...
        self.assertNotContains(history, 'class="history-home-link"')
        self.assertContains(history, 'class="guide-back"')
    ...
    def test_development_mode_exposes_automatic_test_distribution(self):
        ...
        self.assertContains(game, "automaticTestPlayerNames")
        self.assertContains(game, '"Marwen", "Asma", "Marwa", "Mohamed", "Houda"')
        self.assertNotContains(game, '`Test ${String(id).padStart(2, "0")}`')
    ...
    def test_night_summary_sections_and_single_candidate_majority_rule_are_present(self):
        ...
        self.assertContains(game, 'class="accused-card final-vote-card"')
        self.assertContains(game, 'class="final-vote-toggle" name="final-vote-target" type="radio"')
        self.assertContains(game, 'const selectedId = Number(document.querySelector(".final-vote-toggle:checked")?.value || 0)')
        self.assertContains(game, 'state.finalVoteDraftId = Number(event.target.value)')
        self.assertNotContains(game, 'id="final-vote-target"')
        self.assertContains(game, 'alert(L.final_target_required)')
        self.assertContains(game, 'state.voteOutcome = "eliminated"')
        self.assertContains(game, 'state.voteBreakdown = {normal: [], cancelled: [], secret: [], totals: []}')
    ...
    def test_pyromaniac_douses_or_ignites_once_per_night_and_can_be_blocked(self):
        ...
        self.assertContains(game, 'action === "skip-pyromaniac"')
        ...
        self.assertContains(game, 'state.pyromaniacAction = "skipped"')
        self.assertContains(game, 'classList.add("pyromaniac-actions")')
    ...
    def test_player_joins_and_receives_role_after_narrator_starts(self):
        ...
        self.assertContains(tunisian_page, "Chouf role mte3i")
    ...
    def test_night_and_day_history_are_created_once(self):
        ...
        self.assertContains(history_page, "const chainedVoteDeaths")
...
class PwaTests(TestCase):
    def test_manifest_and_service_worker_are_available_at_root_scope(self):
        ...
        self.assertContains(worker, "loup-garou-shell-v18")
        self.assertContains(worker, '"/static/js/theme.js"')
        self.assertContains(worker, 'event.request.destination === "style"')
